...
import { GripVertical, MoreVertical, Copy, Trash2, Edit3 } from "lucide-react";
...
export function PageCard({ page, isSelected, onSelect, onDuplicate, onDelete }: PageCardProps) {
  ...
  // Measure the thumbnail container width for scaling the preview
  useEffect(() => {
    if (thumbnailRef.current) {
      ...
      // Set initial width
      setContainerWidth(thumbnailRef.current.offsetWidth);
    ...
    >
      ...
      <div className="flex items-start gap-2 p-3">
        ...
        >
          ...
          >
            {/* Live page preview */}
            {containerWidth > 0 && <PagePreview pageId={page.id} containerWidth={containerWidth} />}

            {/* Vintage paper texture overlay */}
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_0%,rgba(92,64,51,0.03)_100%)]" />

            {/* Template badge */}
            <div className="absolute right-1 top-1 z-10 rounded-sm bg-sepia/90 px-1.5 py-0.5 text-[9px] font-ui font-medium uppercase tracking-wider text-cream shadow-sm">
              {page.templateId.replace("-", " ")}
            </div>

            {/* Page number badge */}
            <div className="absolute bottom-1 left-1 z-10 flex h-5 w-5 items-center justify-center rounded-full bg-gold/90 text-[10px] font-ui font-bold text-ink shadow-sm">
              {page.order + 1}
            </div>
          </div>
          ...
          {isEditing ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              {isSelected && <Edit3 className="h-3 w-3 flex-shrink-0 text-gold/60" />}
            </div>
          )}
        </button>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-40 border-sepia/20 bg-cream font-ui">
            <DropdownMenuItem
              onClick={(e) => {
                e.stopPropagation();
                setIsEditing(true);
